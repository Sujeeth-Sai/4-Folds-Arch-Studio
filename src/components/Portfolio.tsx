...
function PortfolioCard({ project, idx, onClick }: PortfolioCardProps) {
  const [coords, setCoords] = useState({ x: 0, y: 0 });
  const [isHovered, setIsHovered] = useState(false);

  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    const rect = e.currentTarget.getBoundingClientRect();
    // Calculate mouse position relative to card center, scaled from -0.5 to 0.5
  ...
  return (
    ...
    >
      {/* Cinematic Image Frame with Parallax */}
      <div className="relative w-full overflow-hidden aspect-[4/3] shrink-0">
        <motion.img
          ...
          animate={{
            x: isHovered ? coords.x * -16 : 0,
            y: isHovered ? coords.y * -16 : 0,
